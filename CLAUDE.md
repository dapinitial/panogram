...
## Deploy (shipmate)
- Production = push to `main` on `origin` (`dapinitial/panogram`) — a push publishes (deploy_on_push).
- Only push target: `origin`. After pushing, `git status -sb` must show no "ahead" before claiming a deploy.
- DigitalOcean app: `panogram` · https://panogram-fxfju.ondigitalocean.app (named in `.do/app.yaml`)
- Voice: "roll back panogram, confirm" reverts to the previous deployment; "status" reports the phase.
